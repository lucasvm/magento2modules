...
class Save extends \Magento\Backend\App\Action
{
     ...
     * @see _isAllowed()
     */
    const ADMIN_RESOURCE = 'Lansadigital_Importer::manage_supliers';
        ...
        try {
            ...
            if(isset($data['file_upload']) && !is_null($data['file_upload']))
            {
                ...
                $mediaPath = $this->_filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath()
                     . 'Lansadigital_importer/save/' . $data['file_upload'];
                ...
                foreach ($importProductRawData as $rowIndex => $dataRow) 
                {
                    if($rowIndex > 0) 
                    {
                        $model = $this->_objectManager->create('Lansadigital\Importer\Model\Importer');
    ...
    private function getCSVUploader()
    {
        if ($this->csvUploader === null) {
            $this->csvUploader = \Magento\Framework\App\ObjectManager::getInstance()->get(
                'Lansadigital\Importer\CsvUploader'
            );
        }
